...
struct vec3 
{
    ...
    float angle_between_deg(vec3& v2)
    {
    	...
    	return acos(n / d) * 180.0f / M_PI;
    }

    /*
        Branchless method to find orthogonal vector to any vector.
        
        Citation: 
        Danvil (https://math.stackexchange.com/users/30284/danvil), 
        How to find perpendicular vector to another vector?, 
        URL (version: 2020-03-22): https://math.stackexchange.com/q/3582461
    */
    vec3 get_orthogonal()
    {
        float s = sqrt(x*x + y*y + z*z);
        float g = copysign(s, z);
        float h = z + g;
    ...
    static vec3 min3(const vec3& v1, const vec3& v2, const vec3& v3)
    {
        return vec3(
    ...
    static vec3 max3(const vec3& v1, const vec3& v2, const vec3& v3)
    {
        return vec3(
