...
public:
    ...
    tuple<bool, float> intersects(Ray& ray)
    {
        ...
        */
        ...
        /* no intersection */
        if (d < 0) return tuple<bool, float>{false, -9e15};
    ...
    tuple<bool, float> intersect_caps(vec3& cap, Ray& r)
    {
        ...
        */
        ...
        if (t_cap < 0)
            return tuple<bool, float>{false, -9e15};
    ...
    {
        // float d = x.distanceTo(center);
        // if (d < rad)
        //     return new vec3(this->neg_dir);
        
        // d = x.distanceTo(this->top);
        // if (d < rad)
        //     return new vec3(this->dir);
        ...
        // */
        // vec3* n = new vec3(x - (dir * (dir.dot(x) / dir.lengthSqr())));
        // n->normalize();
        ...
        /* 
            ...
            Get normal vector from that 
        */
        float c = x.distanceTo(this->top);
        c = c*c;
        float b = r_sq;
        float a = sqrt(c-b);

        vec3 p = this->top - (dir*a); 
