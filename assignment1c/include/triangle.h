...
public:
    ...
    tuple<bool, float> intersects(Ray& ray)
    {
        ...
        */
        ...
        if (!can_intersect) return tuple<bool, float>{false, -9e15f};
        ...
        if (t < 0) return tuple<bool, float>{false, -9e15f};
    ...
    vec3 get_normal(vec3& x)
    {
        ...
        */
        ...
        {
            vec3 n;
            n.x = alpha*v0n.x + beta*v1n.x + gamma*v2n.x;
            n.y = alpha*v0n.y + beta*v1n.y + gamma*v2n.y;
            n.z = alpha*v0n.z + beta*v1n.z + gamma*v2n.z;
            n.normalize();

            return n;
        }

        float u = v0t.x*alpha + v1t.x*beta + v2t.x*gamma;
        float v = v0t.y*alpha + v1t.y*beta + v2t.y*gamma;

        vec3 m(norm_map->get_normal_at(u, v));
    ...
    Color* get_diffuse(vec3& x) 
    {
        ...
        /* 
            Find u,v if textured and use 
             those coordinates to get the texture 
